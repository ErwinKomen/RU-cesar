...
var ru = (function ($, ru) {
  ...
  ru.cesar.lingo = (function ($, config) {
    ...
    var loc_example = "",
        ...
        lAddTableRow = [
          ...
          { "table": "research_cond", "prefix": "cond", "counter": true, "events": function () { ru.cesar.lingo.init_cond_events(); } },
          { "table": "research_feat", "prefix": "feat", "counter": true, "events": function () { ru.cesar.lingo.init_feat_events(); } },
          { "table": "result_kwicfilter", "prefix": "filter", "counter": true, "events": null },
          {
            "table": "search_mode_simple", "prefix": "simplerel", "counter": true,
            "events": function () { ru.cesar.lingo.init_simple_events(); }, "follow": function () { ru.cesar.lingo.simple_update(); }
          }
        ];
    ...
    return {
      ...
      ajaxform_click: function () {
        ...
        try {
          ...
          response = ru.cesar.lingo.ajaxcall(ajaxurl, data, "POST");
          ...
          if ('has_errors' in response && response.has_errors) {
            ...
            $(".ajaxform").unbind('click').click(ru.cesar.lingo.ajaxform_click);
            // Stop waiting
          ...
          switch (action) {
            case "save_function":
              ...
              ru.cesar.lingo.get_summary_show("", targeturl, instanceid);
            ...
            default:
              ...
              // Make an AJAX call to get an existing or new specification element HTML code
              response = ru.cesar.lingo.ajaxcall(ajaxurl, data, "GET");
              if (response !== undefined && response.status && response.status === 'ok') {
                ...
                switch (openid) {
                  case "research_container_42":
                    ...
                    ru.cesar.lingo.init_cvar_events();
                    break;
                  ...
                  case "research_container_73":
                    ...
                    ru.cesar.lingo.init_arg_events();
                    break;
                  case "research_container_6":
                    // add any event handlers for wizard part '46'
                    ru.cesar.lingo.init_cond_events();
                    break;
                  case "research_container_7":
                    // add any event handlers for wizard part '46'
                    ru.cesar.lingo.init_feat_events();
                    break;
                }
              }
              break;
      ...
      get_summary_click: function (el, target, bShow) {
        ...
        try {
          ...
          if ($(elSum).hasClass("hidden") || (bShow !== undefined && bShow)) {
            ru.cesar.lingo.get_summary_show(sTargetId, targeturl);
          } else {
            // Hide it
      ...
      get_summary_show: function (target, targeturl, instanceid) {
        ...
        try {
          ...
          response = ru.cesar.lingo.ajaxform_load(targeturl);
          ...
          if (response === undefined || response === null || !("status" in response)) {
            ...
            switch (response.status) {
              ...
              default:
                ...
                setTimeout(function () { ru.cesar.lingo.sync_process(syncurl, synctype, targetdiv); }, 1000);
                break;
            }
          }
        });
      ...
      import_post: function (el) {
        ...
        try {
          ...
          if (syncurl !== undefined && syncurl !== "") {
            ...
            setTimeout(function () { ru.cesar.lingo.sync_process(syncurl, synctype, statusid); }, 2000);
          }
      ...
      init_arg_events: function () {
        try {
          ...
          $(".arg-item").each(function () {
            ...
            ru.cesar.lingo.argtype_click(this);
          });
          // Specify the change reaction function
          $(".arg-type select").change(ru.cesar.lingo.argtype_click);
        } catch (ex) {
      ...
      init_simple_events: function () {
        ...
        try {
          ...
          $('tr td a.delete-row').click(ru.cesar.lingo.tabular_deleterow);

          // Make sure the initial names are stored
          ru.cesar.lingo.simple_store_names();
          ...
          $("tr.rel-form td.rel-name input").on(sEvTypes,
            function () { ru.cesar.lingo.simple_names_update(this, "input"); }
            );

          //$(".delete-row").click(function (elThis) { ru.cesar.lingo.tabular_deleterow(); });
        } catch (ex) {
          ...
          switch (sType) {
            case "input":
              ...
              if (sVarName.indexOf(" ") >= 0) {
                ...
                if (sVarPrev !== "") {
                  ru.cesar.lingo.simple_store_names(sVarPrev, sVarName);
                }
              }
              break;
          }
      ...
      simple_update: function () {
        ...
        try {
          ...
          // Update viewing
          ru.cesar.lingo.related_view(elRelated);
      ...
      init_cond_events: function () {
        try {
          ...
          $(".cond-item").each(function () {
            // Perform the same function as if we were clicking it
            ru.cesar.lingo.condtype_click(this);
          });
          // Specify the change reaction function
          $(".cond-type select").change(ru.cesar.lingo.condtype_click);
          // Specify the function to be called when the user presses "summary"
          $(".func-summary").unbind('click');
          $(".func-summary").click(function () { ru.cesar.lingo.get_summary_click(this, "#cond_summary"); });
        } catch (ex) {
      ...
      init_feat_events: function () {
        try {
          ...
          $(".feat-item").each(function () {
            // Perform the same function as if we were clicking it
            ru.cesar.lingo.feattype_click(this);
          });
          // Specify the change reaction function
          $(".feat-type select").change(ru.cesar.lingo.feattype_click);
          // Specify the function to be called when the user presses "summary"
          $(".func-summary").unbind('click');
          $(".func-summary").click(function () { ru.cesar.lingo.get_summary_click(this, "#feat_summary"); });
        } catch (ex) {
      ...
      init_cvar_events: function () {
        try {
          ...
          $(".cvar-calculate").click(ru.cesar.lingo.cvarcalculate_click);
          // Specify the function to be called when the user presses [Calculation...]
          $(".cvar-specify").click(ru.cesar.lingo.cvarspecify_click);
          // Make sure the 'Type' field values are processed everywhere
          $(".cvar-item").each(function () {
            // Perform the same function as if we were clicking it
            ru.cesar.lingo.cvartype_click(this);
          });
          // Specify the change reaction function
          $(".cvar-type select").change(ru.cesar.lingo.cvartype_click);
          // Specify the function to be called when the user presses "summary"
          $(".func-summary").click(function () { ru.cesar.lingo.get_summary_click(this, "#cvar_summary"); });
      ...
      form_row_select: function () {
        ...
        try {
          ...
          if (bSelected) {
            ...
            ru.cesar.lingo.show_up_down(this, false);
          } else {
            // Select the new row
            $(elRow).addClass("selected");
            // SHow the 'Up' and 'Down' buttons if needed
            ru.cesar.lingo.show_up_down(this, true);
          }
      ...
      init_events: function () {
        try {
          ...
          //$('tr.add-row a').first().click(ru.cesar.lingo.tabular_addrow);
          $("tr.add-row").each(function () {
            $(this).find("a").first().unbind('click').click(ru.cesar.lingo.tabular_addrow);
          });
          $(".delete-row").unbind("click");
          $('tr td a.delete-row').click(ru.cesar.lingo.tabular_deleterow);
          ...
          $('span.td-toggle-textarea').unbind('click').click(ru.cesar.lingo.toggle_textarea_click);
          $('input.td-toggle-textarea').unbind('click').click(ru.cesar.lingo.toggle_textarea_click);
          // Make sure variable ordering is supported
          $('td span.var-down').unbind('click').click(ru.cesar.lingo.var_down);
          $('td span.var-up').unbind('click').click(ru.cesar.lingo.var_up);



          // Attach an event handler to all the NODES
          $("g.main-el").mouseover(function () { ru.cesar.lingo.main_info_show(this) });
          $("g.main-el").mouseout(function () { ru.cesar.lingo.main_info_hide(this) });

          // NOTE: do not use the following mouseout event--it is too weird to work with
          // $('td span.td-textarea').mouseout(ru.cesar.lingo.toggle_textarea_out);

          // Bind the click event to all class="ajaxform" elements
          $(".ajaxform").unbind('click').click(ru.cesar.lingo.ajaxform_click);

          // Activate the EDITABLE interface (from passim)
          $(".ms.editable a").unbind("click").click(ru.cesar.lingo.process_edit);
      ...
      /**
       * init_lingo
       *    Actions to perform when the lingo experiment page is called
       * 
       */
      init_lingo: function () {
        try {
          ...
          $(".xpart").addClass("hidden");
          // Reset the answers
          ru.cesar.lingo.resetAnswers();
          ru.cesar.lingo.resetSurveyAnswers();
          // Make sure we are on top of it
          window.scroll(0, 0);

          // Action for continue button
          $(".continueButton").unbind("click").click(function () { ru.cesar.lingo.exp_button_action("continue");});
      ...
      exp_button_action: function (button_name, el) {
        ...
        try {
          if (button_name !== undefined && button_name != "") {
            switch (button_name) {
              ...
              case "participant_info_old":
                ...
                var englishL1Q = $("[name=englishL1]:checked").length;
                var otherL1Q = $("input[name=otherL1]").val();
                var otherLanguagesQ = $("[name=otherLanguages]:checked").length;
                var educationQ = $("[name=education]:checked").length;
		
		
                if(englishL1Q===0) {
                  $("#warning").html("Please answer if English is your first language!");
                } else if(mQ==="") {
                  $("#warning").html("Please enter your worker ID!");
                } else if(englishL1Q==="N" && otherL1Q==="") {
                  $("#warning").html("Please enter your first language!");
                } else if (ageQ==="" || (!isFinite(ageQ))) {
                  $("#warning").html("Please enter your age in number!");
                } else if (educationQ===0) {
                  $("#warning").html("Please your highest level of education!");
                } else if (genderQ===0) {
                  $("#warning").html("Please enter your gender!");
                } else {
                  loc_profile.englishL1 = $("input[name=englishL1]:checked").val();
                  loc_profile.otherL1 = $("input[name=otherL1]").val();
                  loc_profile.otherLanguages = $("input[name=otherLanguages]").val();
                  loc_profile.aMTurkID = $("[name=mTurkID]").val();
                  loc_profile.age = $("input[name=age]").val();
                  loc_profile.gender = $("input[name=gender]:checked").val();
                  loc_profile.education = $("input[name=education]:checked").val();
                  today = new Date();
                  date = today.getFullYear()+'-'+(today.getMonth()+1)+'-'+today.getDate();
                  time = today.getHours() + ":" + today.getMinutes() + ":" + today.getSeconds();
                  loc_profile.dateTime = date+'_'+time;
	
                  // Possibility: check the mechanical turk ID
                  // requestBackEnd(profile, "/recordProfile");

                  // Hide all qparts and show the first question
                  $(".qpart").addClass("hidden");
          ...
          private_methods.errMsg("exp_button_action", ex);
        }
      },

      /**
       * resetAnswers
       *    Reset radio form buttons to unselected
       * 
       */
      resetAnswers: function () {
        try {
          $("[name=understandQ]").prop('checked', false);
        } catch (ex) {
          private_methods.errMsg("resetAnswers", ex);
        }
      },

      /**
       * resetSurveyAnswers
       *    Reset other input boxes
       * 
       */
      resetSurveyAnswers: function () {
        try {
          $("[name=otherL1]").val("");
          $("[name=otherLanguages]").val("");
          $("[name=englishL1]").prop('checked', false);
          $("[name=gender]").prop('checked', false);
          $("[name=education]").prop('checked', false);
          $("[name=mTurkID]").val("");
          $("[name=age]").val("");
        } catch (ex) {
          private_methods.errMsg("resetSurveyAnswers", ex);
        }
      },
      ...
      kwic_page: function (iPage, sFormDiv) {
        ...
        try {
          elStart = $("#" + sFormDiv);
          ru.cesar.lingo.load_kwic(elStart, iPage);
        } catch (ex) {
      ...
      init_simple: function () {
        ...
        try {
          ...
          switch ($(elTargetType).val()) {
            ...
            case "c":
              ...
              if (iForms > 0) {
                ...
                ru.cesar.lingo.show_related(true);
                bDoRelated = true;
              } else {
                ru.cesar.lingo.show_related(false);
              }
              break;
            case "w":
          ...
          // Check related
          if (bDoRelated) { ru.cesar.lingo.related_view(elRelated); }

          ru.cesar.lingo.init_simple_events();
        } catch (ex) {
      ...
      related_switch: function (elStart, sTarget, sType) {
        ...
        try {
          if ($(elTarget).hasClass("hidden")) {
            ...
            ru.cesar.lingo.simple_modify(elStart);
          }
      ...
      process_item: function (el) {
        ...
        try {
          ...
          if (frm !== undefined) {
            ...
            response = ru.cesar.lingo.ajaxcall(sUrl, data, "POST");
            if (response !== undefined) {
              if (response.status === "ok") {
      ...
      research_watch: function (elThis, sDivName, sDivClose) {
        ...
        try {
          ...
          ru.cesar.lingo.research_open(sDivName, sDivClose);
            ...
            if (response !== undefined) {
              switch (response.status) {
                case "ok":
                  ...
                  setTimeout(function () { ru.cesar.lingo.search_progress(); }, 500);
                  break;
                default:
        ...
        try {
          ...
          switch (sPart) {
            ...
            case "6":
              ...
              response = ru.cesar.lingo.ajaxcall(ajaxurl, data, sAjaxMethod);
              if (response === undefined) {
        ...
        try {
          ...
          switch (sPart) {
            ...
            case "8":
              ...
              // Fetch the corr
              response = ru.cesar.lingo.ajaxform_load(sUrl, sObjectId, data);
              if (response.status && response.status === "ok") {
                // Make the HTML response visible
                $(sTargetId).html(response.html);
                // Make sure events are set again
      ...
      */
          ...
          if (el !== undefined) {
            ...
            if (inst_type !== undefined && inst_type !== "" &&
              ...
              if (sPart !== "") {
                ...
                ru.cesar.lingo.research_wizard(el, sPart, true);
              }
            }
          }
        } catch (ex) {
        ...
        try {
          ...
          switch (sAction) {
            ...
            case "close":   // Hide edit view and enter summary view
              // Check if all is well
              if (ru.cesar.lingo.related_view(elRelated)) {
        ...
        try {
          ...
          $.post(ajaxurl, data, function (response) {
            ...
            if (response.status === undefined) {
              ...
              setTimeout(function () { ru.cesar.lingo.search_progress(after_finish); }, 500);
      ...
      search_progress(after_finish) {
        ...
        try {
          ...
          // Make an AJAX call by using the already stored basket_stop URL
          // response = ru.cesar.lingo.ajaxcall(basket_progress, basket_data, "POST");
        ...
        try {
          ...
          for (i = 0; i < arTdef.length; i++) {
            ...
            if (sId === oTdef.table || sId.indexOf(oTdef.table) >= 0) {
              ...
              rowNew = ru.cesar.lingo.cloneMore(elTable, oTdef.prefix, oTdef.counter);
        ...
        try {
          ...
          if (sTask !== undefined && function_id !== undefined && divTarget !== null) {
            ...
            if (divSave !== null) {
              ...
              ru.cesar.lingo.ajaxform_click.call(divSave);
            }
          }
        } catch (ex) {
